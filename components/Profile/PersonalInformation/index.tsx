import { getUserById, updateAvatar, updateUser } from "@/pages/api";
import {
  Alert,
  AlertColor,
  Box,
  ...
  const fetchDataUserById = async () => {
    if (localStorage.getItem("user")) {
      ...
      if (dataUser) {
        ...
        if (dataUser.data.user.address) {
          setAddress(dataUser.data.user.address);
        }
        if (dataUser.data.user.email) {
          setEmail(dataUser.data.user.email);
        }
        if (dataUser.data.user.phone) {
          setPhone(dataUser.data.user.phone);
        }
        if (dataUser.data.user.sex) {
          if (dataUser.data.user.sex === "male") {
            setSex("1");
          } else {
            setSex("2");
          }
